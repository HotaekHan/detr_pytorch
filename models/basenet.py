...
from models.backbone_hub import load_backbone
...
class BaseNet(nn.Module):
    def __init__(self, config):
        ...
        self.backbone = load_backbone(config, norm_layer=FrozenBatchNorm2d)
        ...
        if config['basenet']['return_interm_layers'] is True:
            return_layers = {"layer1": "0", "layer2": "1", "layer3": "2", "layer4": "3"}
        else:
            return_layers = {'layer4': "0"}
        self.body = IntermediateLayerGetter(self.backbone, return_layers)
...
if __name__ == '__main__':
    ...
    net = build_basenet(config)

    n_parameters = sum(p.numel() for p in net.parameters() if p.requires_grad)
    print('number of params:', n_parameters)

    print(net)
    print("num. of parameters : " + str(n_parameters))
